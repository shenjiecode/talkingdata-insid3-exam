...
用 NumPy 实现 INSID3 §3.1 的位置去偏和跨图匹配。输入是已提取的特征，代码在 CPU 上运行，无需 DINOv3 权重或外部数据集。
...
- 投影后，根据位置基中的非零项，逐通道估计乘加和相减的舍入误差。仅当一个 patch 的所有残差分量均在各自的误差范围内，才将整条残差置零，避免后续归一化形成虚假原型。补零通道不会扩大误差估计；位置基覆盖全部通道时，直接返回零残差。
...
实际结果：`51 passed in 0.12s`。验证环境为 Windows、Python 3.12.8、NumPy 2.5.3、pytest 9.1.1。

`tests/test_insid3_matching.py` 覆盖位置基的维度与正交性、归一化顺序、位置与语义冲突、掩码变化、`rank=0`、非方形网格、空掩码、非法输入及输入不被修改。
数值边界测试还检查一般方向的纯位置特征、覆盖全部通道的位置基、小幅度语义特征，以及补零扩展通道后的匹配结果保持一致。
...
使用 AI 辅助分析、实现、测试和文档撰写；本人已审核并修改相关说明。
